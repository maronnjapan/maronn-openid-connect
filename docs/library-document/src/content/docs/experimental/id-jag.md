...
| ID-JAG draft §9.7（拡張） | `actor_token` と `act` クレーム（`allowActorTokens`、既定 false の opt-in）。`actor_token_type` は RFC 8693 §3 / RFC 7519 §9 の 6 種を一律受理し、内容検証は `actorTokenResolver`（デプロイ側の検証フック）が担う。受領側は act を発行アクセストークンへ引き継ぐ |
...
- `actor_token` を使う場合は `idJagConfig.allowActorTokens` を有効化すること。生成コードの既定リゾルバは「その OP 発行・要求クライアント宛ての ID トークン」だけを有効と判断するので、他の種別を使うなら `idJagConfig.actorTokenResolver` をデプロイ側コードで差し替える（または包んで種別を足す）こと
...
export const idJagConfig = {
  ...
  // 発行側: actor_token の内容検証を担うデプロイ側フック。受理したすべての種別が
  // ここを通る。リクエスト構造と戻り値の構造はライブラリが検証し、トークン内容の
  // 検証はこの関数の責務。既定は自 OP 発行の ID トークンを検証する実装で、
  // undefined にすると actor_token はすべて拒否される
...
`actor_token_type` は RFC 8693 §3 / RFC 7519 §9 が定義する 6 種（`access_token` / `refresh_token` / `id_token` / `jwt` / `saml1` / `saml2`）を種別で区別せず受理し、一覧に無い識別子だけを `invalid_request` で拒否します。
どの種別も同じ `idJagConfig.actorTokenResolver` を通り、そこが「そのトークンが本物か」を決めます。
リクエスト構造（`actor_token` と `actor_token_type` の対応規則、非空、種別が一覧にあるか、opt-in の gate）とリゾルバ戻り値の構造（`sub` 必須、`sub` / `act` 以外の属性の除去）はライブラリが検証し、トークンの内容（署名・失効・帰属）の検証はリゾルバの責務です。
...
生成コードの既定リゾルバは自 OP 発行・要求クライアント宛ての ID トークンだけを有効と判断するので、他の種別を受けるには差し替えるか包みます。

```typescript
// 例: 既定リゾルバ（ID トークン）に、自 OP 発行のアクセストークンを足す
...
idJagConfig.actorTokenResolver = async (input) => {
  if (input.actorTokenType === 'urn:ietf:params:oauth:token-type:access_token') {
    const info = await accessTokenStore.get(input.actorToken);
...
| actor_token の検証失敗（`allowActorTokens` 有効時。`actorTokenResolver` の `null` とリゾルバ未設定）、無効時の actor_token の存在 | `invalid_request`（検証失敗は固定文言） |
| `actor_token_type` が RFC 8693 §3 / RFC 7519 §9 の一覧に無い識別子 | `invalid_request`（受理する一覧を列挙） |
...
- `authorization_details`（RAR）は明示的に拒否します。`actor_token` は `allowActorTokens` を有効化するまで拒否され、`actor_token_type` は RFC 8693 §3 / RFC 7519 §9 の 6 種に限ります。どの actor_token を有効と見なすかは `actorTokenResolver` 次第で、生成コードの既定は本 OP 発行・要求クライアント宛ての ID トークンのみです
- actor と subject の関係性ポリシー（誰が誰の代理を務めてよいか）の判定は持ちません。既定リゾルバでは同じクライアント宛ての有効な ID トークンがあれば actor になれ、それ以上の判定基準はリゾルバ実装者の責務です
