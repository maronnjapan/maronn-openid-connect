...
// --- Helper: コンテキスト作成 ---
// token で既定のアクセストークン情報を部分的に上書きし、user でユーザークレームを差し替える。
function createValidContext(
  overrides?: Partial<UserInfoRequestContext> & {
    token?: Partial<AccessTokenInfo>;
    user?: UserClaims;
  }
): UserInfoRequestContext {
  const { token, user, ...context } = overrides ?? {};
  const tokenInfo = createValidAccessTokenInfo(token);
  const userClaims = user ?? createFullUserClaims();
...
describe('handleUserInfoRequest', () => {
  ...
  describe('Access Token Audience Validation (RFC 9068 §4)', () => {
    ...
    function contextWithAudience(audience: string[] | undefined, expectedAudience?: string) {
      return createValidContext({ token: { audience }, expectedAudience });
    }
  ...
  describe('Required Claims', () => {
    ...
    it('should return sub matching the access token subject', async () => {
      const context = createValidContext({
        token: { sub: 'user-abc' },
        user: createFullUserClaims({ sub: 'user-abc' }),
      });
      const response = await handleUserInfoRequest(context);
...
describe('generateUserInfoJwt', () => {
  ...
  // Helper: 既定の発行者・audience・鍵で署名し、必要なオプションだけを上書きする
  function signUserInfo(
    response: UserInfoResponse = baseResponse,
    overrides: Partial<UserInfoJwtOptions> = {},
  ): Promise<string> {
    return generateUserInfoJwt(response, {
  ...
  describe('JWT structure', () => {
    ...
    it('should include kid in header when keyId is provided', async () => {
      const jwt = await signUserInfo(baseResponse, { keyId: 'key-1' });
      const { header } = decodeJwt(jwt);
  ...
  describe('Required claims', () => {
    ...
    it('should include aud claim matching the client_id', async () => {
      const jwt = await signUserInfo(baseResponse, { audience: 'client-xyz' });
      const { payload } = decodeJwt(jwt);
    ...
    it('should set exp based on expiresIn option', async () => {
      const jwt = await signUserInfo(baseResponse, { expiresIn: 60 });
      const { payload } = decodeJwt(jwt);
  ...
  describe('Additional claims', () => {
    ...
    it('should preserve nested address claim', async () => {
      ...
      const jwt = await signUserInfo(response);
      const { payload } = decodeJwt(jwt);
