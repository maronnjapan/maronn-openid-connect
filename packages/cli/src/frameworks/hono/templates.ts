...
/**
 * Shared, framework-neutral conformance block for the Essential `acr` claim request.
 *
 * OIDC Core 1.0 §5.5.1.1: when `acr` is requested as an Essential Claim with a
 * `value` / `values` member, the OP MUST return a matching `acr` and MUST treat an
 * unmet requirement as a failed authentication attempt. The generated OP's
 * AcrResolver answers with a fixed acr, so requesting a different value as Essential
 * is the observable "cannot be met" case, and the token endpoint must refuse to
 * issue instead of silently returning an ID Token without (or with a weaker) acr.
 *
 * If a user replaces the generated AcrResolver, the second test tells them which
 * value the contract expects; the first is resolver-independent as long as the
 * resolver cannot produce 'urn:example:essential-unreachable'.
 */
export function essentialAcrConformanceBlock(): string {
  return `
 ...
 */
/**
 * Contract tests for the experimental Cross-App Access / ID-JAG feature
 * (draft-ietf-oauth-identity-assertion-authz-grant-04).
 *
 * Emitted only when the id-jag feature is enabled, so the default conformance
 * output is unchanged. Both halves are pinned: issuing an ID-JAG on the
 * token-exchange grant (§4.3) and redeeming one on the jwt-bearer grant (§4.4).
 * The redemption fixtures are signed by a fake external IdP key generated in
 * the block's own beforeAll and trust-listed inline (no network fetch).
 */
