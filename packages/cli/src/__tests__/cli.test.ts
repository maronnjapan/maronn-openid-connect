  ...
  describe('run', () => {
    ...
    describe('feature flags', () => {
      ...
      it('should error on an unknown feature name', () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
          'Error: Unknown feature: "dpop". Available features: pkce, refresh-token, introspection, revocation, request-object. Optional features (disabled by default): transaction-binding. Experimental features (disabled by default): par, token-exchange, jarm, device-authorization-grant, id-jag',
        );
        expect(process.exitCode).toBe(1);
        vi.restoreAllMocks();
        process.exitCode = undefined;
