...
| `token-exchange` | OAuth 2.0 Token Exchange（impersonation のみ） | RFC 8693 | `@maronn-openid-connect/experimental/token-exchange` |
...
`EXPERIMENTAL_FEATURES` に feature-id を追加し、`OidcFeatureConfig` のフラグへ対応づけると、CLI の `--enable` 解決と昇格レビューツール（`pnpm review:experimental`）の対象に自動的に入る。
...
各機能が生成コードへ注入する差分の全文は、機能別の解説に載せている。
機械的に同じ差分を切り出したものが昇格レビューパケット（`tasks/experimental/done/<feature-id>/promotion-review/generated-code/`）にもあり、`pnpm review:experimental <feature-id> --check` で実装との一致を検証できる。
...
判断材料は機能ごとの昇格レビューパケットに集約されていて、生成も更新も `pnpm review:experimental <feature-id>` で行う。
パケットの構成と運用は [tasks/experimental/README.md](../../../tasks/experimental/README.md) を参照。
