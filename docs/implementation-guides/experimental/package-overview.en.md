...
Adding a feature-id to `EXPERIMENTAL_FEATURES` and mapping it to an `OidcFeatureConfig` flag automatically enrolls it in the CLI's `--enable` resolution and in the promotion-review tool (`pnpm review:experimental`).
...
The complete diff that each feature contributes to generated code is embedded in that feature's guide.
The same diffs, cut mechanically, also live in the promotion-review packets (`tasks/experimental/done/<feature-id>/promotion-review/generated-code/`), and `pnpm review:experimental <feature-id> --check` verifies that they still match the implementation.
...
The material for that decision is collected per feature into a promotion-review packet, generated and refreshed with `pnpm review:experimental <feature-id>`.
The packet structure and workflow are described in [tasks/experimental/README.md](../../../tasks/experimental/README.md).
