...
- CLI の `--enable token-exchange` が生成コードへ注入する差分の全文（hono）
...
本実装は **impersonation**（RFC 8693 §1.1 で "the party to whom the token is issued acts as the subject" と説明される型）に限定している。
交換後のトークンの `sub` は subject_token の `sub` のままで、変わらない。
「A が B の代理として振る舞い、その事実をトークンに記録する」delegation（`actor_token` を使う型）は非目標である。
...
- subject_token の解決と有効性検証（本 OP が発行したアクセストークンのみ）
- scope の縮小検証、audience / resource の許可リスト検証、寿命の cap
- §2.2.1 の応答ボディの組み立て

非目標は次のとおりで、いずれも意図的に外している。

- **delegation**（`actor_token` / `actor_token_type`）。提示されたら `invalid_request` で明示的に拒否する
- **アクセストークン以外の token type**。`subject_token_type` も `requested_token_type` も `urn:ietf:params:oauth:token-type:access_token` のみ受け付ける
...
- `sub` は subject_token の値を継承し、変更できない（impersonation 限定の帰結）

もう 1 つの軸はオラクルの排除である。
subject_token の解決失敗（不存在、期限切れ、nbf 未来、失効済み）は、どれも同じ固定文言の `invalid_request` になる。
応答からトークンの存在や失効状況を推測させないための、PAR の `request_uri` と同じ方針である。
クライアント認可（`authorizeTokenExchangeClient`）を最初に置いているのも同じ理由で、許可されていないクライアントには subject_token の有効性判定すらさせない。
...
2. `parseTokenExchangeParams`：必須パラメータと非対応パラメータを検証して型付けする。`actor_token` の拒否、`subject_token` / `subject_token_type` の必須、token type の限定、`resource` の構文検証（絶対 URI かつ fragment なし）を行う。空文字や空白のみの任意パラメータは「送られなかった」と同じに扱い、フォームの空フィールドを黙って対象指定へ昇格させない
3. `resolveSubjectToken`：subject_token を `AccessTokenResolver` で解決し、存在、期限、`nbf` を検証する。失敗理由は応答から区別できない
4. `validateExchangeScope`：要求 scope が subject の scope の部分集合であることを検証し、実効 scope を返す。省略時は subject の scope をそのまま継承する
5. `resolveExchangeTarget`：audience / resource を許可リストで検証する。戻り値は最終的な `aud` ではなく、core の `buildAccessTokenAudience` へ渡す「検証済みの要求」である。両方省略時は subject の audience を継承する（対象変更なしで scope 縮小と期限短縮だけを行う交換として扱う）
6. `computeExchangedTokenLifetime`：寿命を `min(configured, 残存秒数)` で算出する。`resolveSubjectToken` を通過していれば残存秒数は必ず 1 以上になり、`expires_in: 0` のトークンは発行されない
7. `buildTokenExchangeResponse`：§2.2.1 の応答ボディを組み立てる。発行するのは常にアクセストークンなので `token_type` は `Bearer` 固定で、`scope` は判定分岐を避けるため常に含める

合成関数 `processTokenExchangeRequest` はこの 7 つを順に呼び、発行素材 `TokenExchangeGrant` を返す。
`grantId` を subject_token から継承している点は revocation との連動で、grant 単位の失効が交換後のトークンにもそのまま効く。
...
テストは 1 ファイルで、920 行のうち大半をステップ関数ごとの境界値検証が占める。
固定している挙動を要約すると次のとおりである。

- 定数（grant type と token type の URN）と `TokenExchangeError` の性質（常に 400、サニタイズ、エラー名）
- パラメータ解析の全経路（正常系、空白のみの任意パラメータの無視、必須欠落、非対応 token type、delegation の拒否、`resource` の構文違反）
- クライアント認可（URN 未登録、`grantTypes` 未指定の既定、public client の拒否）
- subject_token 解決の全失敗種別が同一文言になること、期限と `nbf` の境界
- scope 縮小（部分集合の受理、超過の拒否、省略時の継承、重複の除去）
- 対象検証（許可リスト内の受理、リスト外の拒否、省略時の subject audience 継承、audience と resource の同時指定）
- 寿命の cap（設定値と残存秒数の min、境界値、設定ミスの `RangeError`）
- 応答ボディの形
- 合成関数の一気通貫（成功時の発行素材、各段階の失敗が正しいエラーで伝播すること）
...
`maronn-oidc generate <framework> --enable token-exchange` を指定すると、生成コードに次が入る。

- **routes/token.ts の変更**：トークンエンドポイントの共有クライアント認証の後、core の `validateGrantTypeSupported` が URN を拒否するより前に、`grant_type === TOKEN_EXCHANGE_GRANT_TYPE` の分岐が入る。分岐内で `processTokenExchangeRequest` を呼び、返った発行素材を core の発行パイプラインへ流して応答を返し切る。`tokenExchangeConfig`（`allowedTargets: []`、既定は安全側の空リスト）もこのファイルに入る
- **routes/discovery.ts の変更**：`grant_types_supported` に交換の URN が加わる
- **conformance.test.ts の変更**：交換の成功と失敗の挙動一式を契約テストとして固定する

トークンエンドポイントに新しいルートは増えない。
既存の `POST /token` の中に grant の分岐が 1 つ増える、という形の統合である。

### 生成コードに入る差分の全文（hono）

以下は、デフォルト構成（`default-op`）と `--enable token-exchange`（`with-token-exchange`）の生成結果の差分そのもので、`--enable token-exchange` が生成コードに足すもののすべてである。
...
E2E テストは、ブラウザで Authorization Code Flow を完走して取得した実物のアクセストークンを subject_token に使い、次を固定する。

- 交換の成功（scope を狭めたトークンが返ること）
- discovery が交換の URN を広告すること
- クライアント認証なしの交換の拒否
- 未知の subject_token が `invalid_request` になること
- `actor_token` を伴う delegation 要求の拒否

discovery が交換の URN を広告しない OP では、各テストはスキップされる。
...
- 仕様検討文書：[tasks/experimental/done/token-exchange/](../../../tasks/experimental/done/token-exchange/)
