...
- 🟠 **fail-open な既定値**: `action` が欠落・空文字・未知値のとき承認になる。具体的に到達しうる経路:
  - フォームを Enter キーで送信した際、送信に使われた submit ボタンが特定できず `action` が付かない
    ブラウザ／UA（歴史的に挙動差がある）。この場合ユーザーは何もボタンを押していないのに承認される。
  - 利用者が view をカスタマイズして `value="allow"` / `value="accept"` に変えた場合、**拒否ボタンだけが
    壊れずに残り、承認は「意図せず常に成立」する**。誤りが画面上で観測できないため気づけない。
  - 自動化テスト・スクレイパ・プリフェッチ等が `csrf_token` を含むフォームを機械的に POST した場合。
