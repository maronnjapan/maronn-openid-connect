...
|---|---|---|---|---|---|---|---|
| OpenID Connect Client-Initiated Backchannel Authentication Flow - Core 1.0 | OpenID Foundation | https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html | Final Specification | §4, §7.1, §7.1.1, §7.1.2, §7.2, §7.3, §10.1, §10.3.1, §11, §13, §14, §15 | 認証リクエストのパラメータ定義とヒント規則（one and only one）、auth_req_id のエントロピー（最低 128bit・推奨 160bit）と文字種（A-Z a-z 0-9 . - _）、Poll モードの grant URN と状態機械エラー語彙、slow_down の恒久 +5 秒、discovery/registration メタデータの REQUIRED/OPTIONAL、user_code の脅威モデル、login_hint の PII 性 | 2026-08-08 | 1.0 Final (2021-09-01) |
...
- **設計判断（仕様が余地を残す部分）**: 公開クライアント（auth method `none`）の `unauthorized_client` 拒否 / 非対応ヒント種別への `invalid_request` / `requested_expiry` のクランプ（§7.1 は MAY）/ 過剰ポーリングへ `invalid_request`（§11 の MAY）ではなく slow_down 方式で統一 / `maxPendingPerSubject` 上限と超過時 `invalid_request` / denied レコードの access_denied 配信時削除 / `backchannelTokenDeliveryMode` 未設定を poll とみなす / user_code・署名リクエスト・Ping/Push の非対応
