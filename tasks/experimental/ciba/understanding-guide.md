...
2. **承認の乗っ取り**: `auth_req_id` を知る攻撃者が承認を偽造する脅威。承認操作は「認証済み OP セッションの subject = レコードの subject」でなければ実行できないため、`auth_req_id` の知識だけでは何もできない。Device Flow で必要だった binding Cookie が CIBA に無いのはこのため（Device Flow は user_code しかリンクが無いが、CIBA はレコード自体がユーザーを指名している）
3. **`auth_req_id` の窃取**: トークン取得にはクライアント認証と「発行先クライアント一致」（`invalid_grant`）が要るため、`auth_req_id` 単独ではトークンを取れない。それでも authorization code と同格の機密として扱い、ログへ出さない
4. **トランザクション取り違え**: オペレーターの画面と本人のスマホで別件を承認してしまう脅威。`binding_message` を両画面に表示する視覚照合が仕様上の対策（§7.1）
5. **ユーザー列挙**: `unknown_user_id` はユーザーの存在有無を登録クライアントに教える。これは仕様が定義する語彙（§13）であり返すが、error_description は固定文言にし、匿名アクセスはクライアント認証で遮断する
...
- `userCode` / `bindingHash` が無い（ユーザーはコードを入力せず、セッション subject 一致で束縛する）
